...
class ArtistMyPageHandler(LoginBaseHandler):
    def get(self):
        ...
        self.render("mypageArtist.html", user=results, messages=messages, errors=[])
...
class CashbookCreateHandler(SigninBaseHandler):
    def get(self):
        ...
        art = user.__init__()
        self.render("signupArtist.html", user=art, mode="new", messages=[], errors=[])
    ...
    def post(self):
        ...
        if p_email is None: errors.append("メールアドレスは必須です。")
        art.attr["email"] = p_email
        
        if p_password is None: errors.append("パスワードは必須です。")
        art.attr["password"] = p_password

        if p_fan_class is None: errors.append("ファンの年齢層を入れてください。")
        art.attr["fan_class"] = Decimal(p_fan_class)

        if p_sex is None: errors.append("ファンの性別は必須です。")
        art.attr["sex"] = Decimal(p_sex)

        if p_genre is None: errors.append("音楽のジャンルは必須です。")
        ...
        art_id = art.save()
        if art_id == False:
            self.render("signupArtist.html", user=art, mode="new", messages=[], errors=["登録時に致命的なエラーが発生しました。"])
        else:
            # 登録画面へリダイレクト(登録完了の旨を添えて)
            self.redirect("/signupArtist.html?message=%s" % tornado.escape.url_escape("新規登録完了しました。(ID:%s)" % art_id))
